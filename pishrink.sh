...
do_expand_rootfs() {
  ...
  ROOT_PART_NUM=$(echo $ROOT_PART|sed -e 's/.*[^0-9]\([0-9]\+\)[^0-9]*$/\1/')
  ROOT_DEV=/dev/$(lsblk -no pkname $ROOT_PART)

  # Get the starting offset of the root partition
  PART_START=$(parted $ROOT_DEV -ms unit s p | grep "^${ROOT_PART_NUM}" | cut -f 2 -d: | sed 's/[^0-9]//g')
  [ "$PART_START" ] || return 1
...
if [[ $(findmnt / -o source -n | grep "/dev/mmcblk") != "" ]]; then
  raspi_config_expand
fi
